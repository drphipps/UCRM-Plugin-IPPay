<?php
/*
 * @copyright Copyright (c) 2018 Ubiquiti Networks, Inc.
 * @see https://www.ubnt.com/
 */

declare(strict_types=1);

namespace AppBundle\Grid\InvoiceTemplate;

use AppBundle\Component\Grid\Grid;
use AppBundle\Component\Grid\GridFactory;
use AppBundle\Component\Grid\GridHelper;
use AppBundle\Controller\InvoiceTemplateController;
use AppBundle\DataProvider\InvoiceTemplateDataProvider;
use AppBundle\Security\Permission;
use AppBundle\Util\Formatter;

class InvoiceTemplateGridFactory
{
    /**
     * @var GridFactory
     */
    private $gridFactory;

    /**
     * @var GridHelper
     */
    private $gridHelper;

    /**
     * @var InvoiceTemplateDataProvider
     */
    private $invoiceTemplateDataProvider;

    public function __construct(
        GridFactory $gridFactory,
        GridHelper $gridHelper,
        InvoiceTemplateDataProvider $invoiceTemplateDataProvider
    ) {
        $this->gridFactory = $gridFactory;
        $this->gridHelper = $gridHelper;
        $this->invoiceTemplateDataProvider = $invoiceTemplateDataProvider;
    }

    public function create(): Grid
    {
        $qb = $this->invoiceTemplateDataProvider->getGridModel();
        $grid = $this->gridFactory->createGrid($qb, __CLASS__);
        $grid->setRowUrl('invoice_template_show');
        $grid->addIdentifier('it_id', 'it.id');
        $grid->addIdentifier('it_official_name', 'it.officialName');
        $grid->setDefaultSort('it_name');

        $grid->attached();

        $grid->addTextColumn('it_name', 'it.name', 'Name')->setSortable();
        $grid
            ->addTwigFilterColumn(
                'it_created_date',
                'it.createdDate',
                'Created date',
                'localizedDate',
                [Formatter::DEFAULT, Formatter::NONE]
            )
            ->setSortable();

        $grid
            ->addEditActionButton('invoice_template_edit', [], InvoiceTemplateController::class)
            ->addRenderCondition(
                function ($row) {
                    return ! $row['it_official_name'];
                }
            );

        $cloneButton = $grid->addActionButton(
            'invoice_template_clone',
            [],
            InvoiceTemplateController::class,
            Permission::EDIT
        );
        $cloneButton->setIcon('ucrm-icon--plus-recurring');
        $cloneButton->setData(
            [
                'tooltip' => $this->gridHelper->trans('Clone'),
            ]
        );

        $grid
            ->addDeleteActionButton('invoice_template_delete', [], InvoiceTemplateController::class)
            ->addRenderCondition(
                function ($row) {
                    return ! $row['it_official_name'];
                }
            );

        return $grid;
    }
}
